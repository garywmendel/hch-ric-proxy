...
async function fetchQuickBooks(startDate, endDate) {
  ...
  const income = {
    ...
    banquet_wine: acct(raw,"4198"), banquet_rentals: acct(raw,"4195"),
      sales_refund: acct(raw,"4160"), transport_fee: acct(raw,"4200"), discounts_comps: acct(raw,"4500"),
  };
  income.total_sales = sum(
    ...
    income.banquet_rentals, income.sales_refund, income.transport_fee, income.discounts_comps
  );

  const cogs = {
    food_base: acct(raw,"5100"), meat: acct(raw,"5101"), produce: acct(raw,"5103"), grocery: acct(raw,"5104"),
  ...
  cogs.food  = sum(cogs.food_base, cogs.meat, cogs.produce, cogs.grocery, cogs.bakery, cogs.dairy);
  ...
  const ga_expenses = {
    ...
    consulting: acct(raw,"6510"), recruiting: acct(raw,"6512"), legal: acct(raw,"6520"),
    ...
    filing_fees: acct(raw,"6565"), license_permits: acct(raw,"6570"), office_supplies: acct(raw,"6575"),
    postage: acct(raw,"6585","6603"), liability_insurance: acct(raw,"6590"),
    do_insurance: acct(raw,"6595"), penalties_settlements: acct(raw,"6600"),
    phone_internet: acct(raw,"6605"),
  };
  ga_expenses.total = sum(
    ga_expenses.research_development, ga_expenses.accounting_bookkeeping, ga_expenses.consulting,
    ga_expenses.recruiting, ga_expenses.legal, ga_expenses.payroll_processing,
    ga_expenses.computer_software_it, ga_expenses.dues_subscriptions, ga_expenses.bank_charges,
    ga_expenses.filing_fees, ga_expenses.license_permits, ga_expenses.office_supplies,
    ga_expenses.postage, ga_expenses.liability_insurance, ga_expenses.do_insurance,
  ...
  const repair_maintenance = { equipment: acct(raw,"6810"), facility: acct(raw,"6815"), pest_control: acct(raw,"6820"), fire_control: acct(raw,"6825"), facility_supplies: acct(raw,"6835") };
  repair_maintenance.total = sum(repair_maintenance.equipment, repair_maintenance.facility, repair_maintenance.pest_control, repair_maintenance.fire_control, repair_maintenance.facility_supplies);
  ...
  const other_expenses = { other_income_expense: acct(raw,"8130"), taxes_fees: acct(raw,"8140"), corporate_overhead: acct(raw,"8510") };
  other_expenses.total = sum(other_expenses.other_income_expense, other_expenses.taxes_fees, other_expenses.corporate_overhead);
...
// Apple touch icon for home screen — star-guitar mark on black background
